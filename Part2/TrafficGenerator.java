...
class TrafficGenerator extends Thread {  
	...
	public void run () { 
		try {  
			...
			// r is in kbps, transform to the bytes per second
			long rInBps = r * 1000 / 8; 
			// calculate each packet size
			long packetSizeInByte = L/N; 
			// calculate the time interval
			long intervalInNanosec = (packetSizeInByte * 1000*1000*1000)/ rInBps;
			System.out.println("Packet Size: " + packetSizeInByte + " Bytes per packet");
			System.out.println("Time interval: " + intervalInNanosec + " nanosecond");
			System.out.println("Num of Packets: " + N);

			long lastSendTime = System.nanoTime();
			long timestamp = 0;

			for (int i = 1; i <= N; i++){ // seqno starts at 1
			    ...
			    // set timestamp
			    if (i == 1) { // special handle for the first case
